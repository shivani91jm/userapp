...
enum ServiceTabControllerState {serviceOverview,faq,review,gallery}
...
class ServiceTabController extends GetxController with GetSingleTickerProviderStateMixin{
  ...
  List<Widget> serviceDetailsTabs(){
    if(faqs!.isNotEmpty){
      return  [
        ...
        Tab(child: Text("gallery".tr,maxLines: 2,),)
      ];
    }
    return  [
      Tab(child: Text("service_overview".tr,maxLines: 2,),),
  ...
  void onInit() {
    super.onInit();
    controller = TabController(vsync: this, length: faqs!.isNotEmpty ? 4 :3);
  }
